...
def training(args, dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):   
        ...
        # DropGaussian: 计算当前的drop rate
        current_drop_rate = 0.0
        if args.use_dropgaussian:
            if args.drop_schedule == "linear":
                # 线性调度：从0逐渐增加到drop_rate
                current_drop_rate = args.drop_rate * (iteration / opt.iterations)
            elif args.drop_schedule == "cosine":
                # 余弦调度：更平滑的变化
                current_drop_rate = args.drop_rate * (1 - np.cos(np.pi * iteration / opt.iterations)) / 2
            elif args.drop_schedule == "constant":
                # 常数调度：固定drop rate
                current_drop_rate = args.drop_rate
            else:
                current_drop_rate = args.drop_rate
        ...
        render_pkg = render(viewpoint_cam, gaussians, pipe, bg, 
                          is_train=True, iteration=iteration, drop_rate=current_drop_rate)
        ...
        with torch.no_grad():
            ...
            if iteration % 10 == 0:
                # 在进度条中显示DropGaussian信息
                drop_info = f"drop:{current_drop_rate:.3f}" if args.use_dropgaussian else ""
                progress_bar.set_postfix({'Loss': f"{ema_loss_for_log:.{7}f}", 'n': f"{num_gauss}", 'DropGaussian': drop_info})
...
if __name__ == "__main__":
    ...
    parser = ArgumentParser(description="Training script parameters with DropGaussian")
    ...
    # DropGaussian 相关参数
    parser.add_argument("--use_dropgaussian", action="store_true", default=False,
                        help="Enable DropGaussian regularization")
    parser.add_argument("--drop_rate", type=float, default=0.1,
                        help="DropGaussian drop rate (0.0 to 1.0)")
    parser.add_argument("--drop_schedule", type=str, default="linear",
                        choices=["linear", "cosine", "constant"],
                        help="DropGaussian schedule: linear, cosine, or constant")
    ...
    # 打印DropGaussian配置信息
    if args.use_dropgaussian:
        print(f"DropGaussian enabled: drop_rate={args.drop_rate}, schedule={args.drop_schedule}")
    else:
        print("DropGaussian disabled")
